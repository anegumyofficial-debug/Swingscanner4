...
def run_mega_scanner(ticker_list):
    ...
    def get_decision_strategy(trend, action):
    # Logika Detail Matriks Keputusan
    if "Up-Trend" in trend:
        if "SUPER BUY" in action: return "Aggressive Buy (Accumulate)"
        if "BUY" in action: return "Buy on Weakness"
        return "Hold / Watch"
    elif "Sideways" in trend:
        if "SUPER BUY" in action: return "Buy & Scalp (TP Pendek 1-3%)"
        if "BUY" in action: return "Range Trading (Buy @ Support)"
        return "Skip / Wait & See"
    elif "Down-Trend" in trend:
        if "SUPER BUY" in action: return "Speculative Buy (Small, Strict SL)"
        if "BUY" in action: return "Wait / Cautious (Bull Trap Risk)"
        return "Avoid / Exit Strategy"
    return "Neutral"
...
if len(saham_pilihan) > 0:
    ...
    if not df_radar.empty:
        ...
        # --- TABEL MATRIKS KEPUTUSAN DETAIL ---
        st.markdown("### 📋 Matriks Pengambilan Keputusan (Tren vs Sinyal)")
        
        # Membuat dataset matriks lengkap
        matriks_data = {
            "Tren": ["Up-Trend", "Up-Trend", "Up-Trend", "Sideways", "Sideways", "Sideways", "Down-Trend", "Down-Trend", "Down-Trend"],
            "Sinyal": ["SUPER BUY", "BUY (Oversold)", "RISK", "SUPER BUY", "BUY (Oversold)", "RISK", "SUPER BUY", "BUY (Oversold)", "RISK"],
            "Tindakan Utama": [
                "Aggressive Buy (Accumulate)", "Buy on Weakness", "Hold / Profit Taking",
                "Buy & Scalp (TP Pendek)", "Range Trading (Buy @ Support)", "Sell / Exit",
                "Speculative Buy (Small)", "Wait / Cautious", "Avoid / Exit Strategy"
            ]
        }
        df_matriks = pd.DataFrame(matriks_data)
        
        # Menampilkan tabel dengan gaya agar mudah dibaca
        st.table(df_matriks)
        
        st.info("""
        💡 **Tips:** Gunakan kolom 'Tindakan Utama' di atas untuk memfilter psikologi trading Anda. 
        Jangan pernah melawan arah tren besar (Down-Trend) kecuali menggunakan dana spekulasi yang sangat kecil.
        """)        
        st.info("""
